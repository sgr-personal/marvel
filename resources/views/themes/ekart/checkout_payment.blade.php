                                                    ...
                                                    @foreach(Cache::get('timeslot')->slots as $slot)
                                                        @if($slot->status == 1)
                                                            <label class="custom-control custom-radio">
                                                                <input class="custom-control-input" type="radio" name="deliverTime" value="{{ $slot->title }}" data-from="{{ $slot->from_time }}" data-to="{{ $slot->to_time }}" data-last="{{ $slot->last_order_time }}">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="">
                                <tr class="mr-5">
                                    <td>                               
                                        <p class="product-name">{{__('msg.subtotal')}} : <span>{{ $data['subtotal'] ?? '-' }}</span></p>
                                        @if(isset($data['tax_amount']) && floatval($data['tax_amount']))
                                            <p class="product-name">{{__('msg.tax')}} {{ $data['tax'] }}% : <span>+ {{ get_price($data['tax_amount']) }}</span></p>
                                        @endif
                                        ...
                                        @endif
                                        <p class="product-name">{{__('msg.total')}} : <span> {{ $data['total'] }}</span></p>                                       
                                    </td>
                                </tr>                                
                                <tr class="text-left">
                                    <td>
                                        <strong>
                                            <p class="checkout-total walletNotUsed">{{ __('msg.final_total')}} : <span>{{ $data['total'] }}</span></p>
                                            @if(intval($data['user']['balance'] ?? 0))
                                                @if(floatval($data['user']['balance']) > floatval($data['total']))
                                                    <p class="product-name walletUsed">{{__('msg.wallet_from')}} : <span>{{ floatval($data['total']) }}</span></p>
                                                    <p class="checkout-total walletUsed">{{__('msg.total_payable')}} : <span> 0</span></p>
                                                @else
                                                    <p class="product-name walletUsed">{{__('msg.wallet_from')}} : <span>- {{ $data['user']['balance'] ?? '0' }}</span></p>
                                                    <p class="checkout-total walletUsed">{{__('msg.total_payable')}} : <span> {{ floatval($data['total']) - floatval($data['user']['balance']) }}</span></p>
                                                @endif
                                            @endif
                                        </strong>
                                    </td>
                            ...
                            <div class="row add-to-fav1 mr-4">
                                ...
                                <form action="{{ route('checkout-proceed') }}" method="POST">
                                    ...
                                    @if(intval($data['user']['balance'] ?? 0))
                                        ...
                                        @else
                                            <input type="hidden" name="wallet_balance" value="{{ floatval($data['total']) - floatval($data['user']['balance']) }}">
                                        @endif
                                    @else
