...
class HomeController extends Controller
{
    ...
    public function get_shop_params_category(Request $request)
    {
            ...
            if (isset($categories[$cat]->childs)) {
                ...
                foreach ($selectedSubCategory as $sub) {
                    if (isset($childs[$sub])) {
                        ...
                        if (isset($childs[$sub]->attribute_ids) && $childs[$sub]->attribute_ids != '') {
                            $attr_ids .= $childs[$sub]->attribute_ids . ',';
                        }
                    }
                }
            }
        ...
        $selectedAttrs = explode(",", $request->get('attribute-id', ""));
        $selectedAttrValues = explode(",", $request->get('attribute-value', ""));
        $attributes = [];
        $attr_ids = rtrim($attr_ids, ',');
        if ($attr_ids != '') {
            $attributes = $this->post('get-attributes', ['data' => [api_param('ids') => $attr_ids]]);
            foreach ($selectedAttrs as $attr) {
                if (isset($attributes[$attr])) {
                    $selectedAttrIds[intval($attributes[$attr]->id ?? 0)] = intval($attributes[$attr]->id ?? 0);
                    $values = (array)$attributes[$attr]->values;
                    foreach ($selectedAttrValues as $attr_value) {
                        if (isset($values[$attr_value])) {
                            unset($selectedAttrIds[intval($attributes[$attr]->id ?? 0)]);
                            $selectedAttrValueIds[] = intval($values[$attr_value]->id ?? 0);
                        }
                    }
                }
            }
        ...
        return ['selectedCategory' => $selectedCategory, 'selectedCategoryIds' => $selectedCategoryIds, 'selectedSubCategory' => $selectedSubCategory, 'selectedSubCategoryIds' => $selectedSubCategoryIds, 'selectedAttrs' => $selectedAttrs, 'selectedAttrValues' => $selectedAttrValues, 'selectedAttrIds' => $selectedAttrIds, 'selectedAttrValueIds' => $selectedAttrValueIds, 'attributes' => $attributes];
    }
    ...
    public function get_shop_params(Request $request)
    {
        ...
        $param['attribute-id'] = trim($request->{'attribute-id'});
        $param['attribute-value'] = trim($request->{'attribute-value'});
        $data['category'] = implode(",", $selectedCategoryIds);
        $data['sub-category'] = implode(",", $selectedSubCategoryIds);
        $data['attribute-id'] = implode(",", $selectedAttrIds);
        $data['attribute-value'] = implode(",", $selectedAttrValueIds);
        ...
        return ['data' => $data, 'param' => $param, 'page' => $page, 'limit' => $limit, 'selectedCategory' => $selectedCategory, 'selectedSubCategory' => $selectedSubCategory, 'selectedAttrs' => $selectedAttrs, 'selectedAttrValues' => $selectedAttrValues, 'attributes' => $attributes];
    }
